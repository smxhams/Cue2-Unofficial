...
public sealed class AudioSourceDecoder : IDisposable
{
    ...
    private unsafe void CloseInternal()
    {
        ...
        if (_pcmStore != null)
        {
            released += Cue2.Shared.Audio.MediaMemory.FloatBufferBytes(_pcmStore);
            _pcmStore = null;
        }
        if (_convertScratch != null)
        {
            released += Cue2.Shared.Audio.MediaMemory.FloatBufferBytes(_convertScratch);
            _convertScratch = null;
        }
        _pcmFrameCount = 0;
        _pcmReadFrame = 0;
        ...
        _discardOutputFrames = 0;
        if (released > 0)
            Cue2.Shared.Audio.MediaMemory.NoteReleased(released);
