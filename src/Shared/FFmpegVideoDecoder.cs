...
/// Handles embedded audio by syncing with a separate audio decoder if present.
/// </summary>
public class FFmpegVideoDecoder : IDisposable
{
    ...
    private FFmpegAudioDecoder _audioDecoder; // If video has audio, sync with this
    private bool _hasAudio = false;

    /// <summary>
    /// Initializes a new instance of the <see cref="FFmpegVideoDecoder"/> class.
    /// </summary>
    ...
    public FFmpegVideoDecoder(VideoComponent component, ActiveVideoPlayback playback)
    {
        ...
        // Check for embedded audio and setup sync
        if (_component.Metadata.AudioChannels > 0)
        {
            _hasAudio = true;
            // Create audio component from video metadata for sync
            var audioComponent = new AudioComponent
            {
                AudioFile = _component.VideoFile,
                StartTime = _component.StartTime,
                EndTime = _component.EndTime,
                Volume = _component.Volume,
                Loop = _component.Loop,
                PlayCount = _component.PlayCount,
                Metadata = new AudioFileMetadata
                {
                    Duration = _component.Metadata.Duration,
                    Channels = _component.Metadata.AudioChannels,
                    SampleRate = _component.Metadata.AudioSampleRate,
                    BitDepth = _component.Metadata.AudioBitDepth,
                    Codec = _component.Metadata.AudioCodec,
                    Format = _component.Metadata.Format
                }
            };
            _audioDecoder = new FFmpegAudioDecoder(audioComponent, null); // No ActiveAudioPlayback for sync; handle separately
        }
    }
    ...
    public async Task InitAsync()
    {
        await Task.Run(() =>
        {
            unsafe
            {
                ...
                // Init audio decoder if present
                if (_hasAudio)
                {
                    Task.Run(() => _audioDecoder.InitAsync()).Wait();
                }
    ...
    public void Pause()
    {
        ...
        _pauseEvent.Reset();
        if (_hasAudio) _audioDecoder.Pause();
    }
    ...
    public void Resume()
    {
        ...
        _pauseEvent.Set();
        if (_hasAudio) _audioDecoder.Resume();
    }
    ...
    public void Stop()
    {
        ...
        _pauseEvent.Set();
        if (_hasAudio) _audioDecoder.Stop();
    }
    ...
    public void Seek(long timestampUs)
    {
        ...
        if (_hasAudio) _audioDecoder.Seek(timestampUs);
    }
