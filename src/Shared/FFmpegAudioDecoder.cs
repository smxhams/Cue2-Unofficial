        ...
        Task.Run(() =>
        {
            unsafe
            {
                ...
                try
                {
                    ...
                    var effectivePlaycount = _component.Loop ? int.MaxValue : _component.PlayCount;
                    while (!_isStopped && (playCount < effectivePlaycount)  && !_cts.IsCancellationRequested) // (handle PlayCount, Loop via infinite if Loop)
                    {
