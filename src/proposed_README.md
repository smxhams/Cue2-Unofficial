...
Cue2 is a cross-platform event playback and show control application, inspired by QLab. Built with Godot 4.7.1 Mono, FFmpeg.AutoGen 9.0.1.1, SDL3-CS 3.4.14.1, and official RtMidi 6.0, it supports audio/video playback, OSC, MIDI device I/O, text overlays, session management, and more. Targets Windows 10+, macOS, and Linux.

## Features
- Audio and video playback (wide format support via FFmpeg)
- OSC send/receive for integration
- MIDI input (triggers / Input Map) and MIDI output components
...
2. Install Godot 4.7.1 Mono from [godotengine.org](https://godotengine.org/download).
3. Open `Cue2.sln` in your IDE or use Godot editor to import the project.
4. Ensure FFmpeg, SDL, and RtMidi natives are present under `bin/` (`python tools/build-rtmidi-natives.py` rebuilds MIDI libs).
...
- FFmpeg.AutoGen (MIT)
- SDL3-CS (zlib)
- RtMidi 6.0 (MIT-style) — MIDI device I/O; natives in `bin/`
...
## Credits
Built on Godot Engine (MIT), FFmpeg (LGPL), SDL (zlib), RtMidi (MIT-style).